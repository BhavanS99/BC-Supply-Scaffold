...
contract YourContract is AccessControl {
   ...
   //Define Roles
    ...
    struct itemModular {
        ...
        uint[]   components;             // store SKU's of parts
    }
    ...
    mapping(uint => uint)       public _costs;              // keep track of costs of modular parts
        ...
        */
        ...
        if (productCode != 6) {
            if (productCode == 4) {
                ...
                for (uint i = 0; i < _costs[4]; i++ ) {
                    uint s = metalQ[metalQ.length -1].sku;
                    comp[i] = s;
                    metalQ.pop();
                }
            }
                ...
                for (uint i = 0; i <_costs[7]; i++ ) {
                    uint s = rubberQ[rubberQ.length -1].sku;
                    comp[i] = s;
                    rubberQ.pop();
                }
            }
            ...
            for (uint i = 0; i < _costs[6]; i++ ) {
                    uint s = plasticQ[plasticQ.length -1].sku;
                    comp[i] = s;
                    plasticQ.pop();
                }
        }
    ...
    function produceCar(
        ...
        // create a car item
        require(interiorQ.length >= 1 &&
                 chassisQ.length >= 1 &&
                  engineQ.length >= 1 &&
                   wheelQ.length >= 1);
        // get car components
        uint x = interiorQ[interiorQ.length -1].sku;
        uint y = chassisQ[chassisQ.length -1].sku;
        uint z = engineQ[engineQ.length -1].sku;
        uint w = wheelQ[wheelQ.length -1].sku;
        uint[] memory comp = new uint[](4);
        interiorQ.pop();
        chassisQ.pop();
        engineQ.pop();
        wheelQ.pop();
